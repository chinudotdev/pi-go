# pi-go

Go SDK for building AI-powered agents — provider-agnostic, streaming-first, with tool calling, session persistence, and multi-turn conversations.

## Where to Look

| Want to… | Go here |
|----------|--------|
| Understand the design | [`ARCHITECTURE.md`](ARCHITECTURE.md) |
| See how packages fit together | [`ARCHITECTURE.md`](ARCHITECTURE.md) — package map |
| Browse runnable examples | [`examples/`](examples/) |
| Read about core AI types & providers | [`ai/README.md`](ai/README.md) |
| Read about the agent loop & tools | [`agent/README.md`](agent/README.md) |
| Read about the SDK layer | [`sdk/README.md`](sdk/README.md) |

## Quick Install

```bash
go get github.com/chinudotdev/pi-go/agent
```

## License

[MIT](LICENSE) © chinudotdev
